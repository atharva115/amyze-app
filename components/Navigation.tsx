...
export const BottomNav: React.FC<NavProps> = ({ currentPage, setPage, userRole, isLoggedIn }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-3 px-6 flex justify-between items-center z-50 md:hidden shadow-lg rounded-t-xl">
...
export const Sidebar: React.FC<NavProps> = ({ currentPage, setPage, userRole, isLoggedIn, logout }) => {
  if (!isLoggedIn) return null;
  ...
  return (
    <div className="hidden md:flex flex-col w-64 bg-white h-screen fixed left-0 top-0 border-r border-gray-200 p-6 z-40">
      ...
      <button onClick={logout} className="mt-auto flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 rounded-xl transition-all">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
